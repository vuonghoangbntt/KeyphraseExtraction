...
model = NewModel(vocab_size=len(vocab.keys()), emb_size=int(args.glove_size), hidden_size=args.hidden_size, output_size=len(output_tag.keys()), embedding=embedding, use_pretrain=args.use_pretrain, num_layers=args.num_layers).to(device)

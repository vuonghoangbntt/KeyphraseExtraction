...
model = BERT_BiLSTM_CRF( hidden_size=args.hidden_size, output_size=len(output_tag.keys()), num_layers=args.num_layers).to(device)
...
for epoch in range(args.num_epoch):
    ...
    if (epoch+1) % args.num_epoch_save == 0:
        checkpoint = {'model.lstm': model.lstm.state_dict(),'model.hidden2tag': model.hidden2tag.state_dict(), 'model.crf': model.crf.state_dict(), 'optimizer': optimizer.state_dict()}
